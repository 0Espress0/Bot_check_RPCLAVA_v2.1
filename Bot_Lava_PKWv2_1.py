...
web3 = None  # Initialize web3 variable
...
def toggle_wallet_connection(private_key, rpc_endpoint):
    global wallet_connected, web3
    try:
...
def check_rpc_status(web3):
    try:
        if web3 is not None:
            ...
            print("Web3 is not initialized. Please check the connection.")
    except Exception as e:
        print("Error while checking node status:", e)

def check_wallet_balance(web3):
    try:
        if web3 is not None:
            if wallet_connected:
...
while True:
    ...
    if rpc_endpoint and private_key:
        toggle_wallet_connection(private_key, rpc_endpoint)  # Connect or disconnect wallet
        
        # Check if web3 is initialized before using it
        if web3 is not None:
            check_rpc_status(web3)  # Check RPC node status
            check_wallet_balance(web3)  # Check wallet balance
        else:
            print("Web3 is not initialized. Please check the connection.")
